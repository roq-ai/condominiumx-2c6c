...
function PropertyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PropertyInterface>(
    ...
    () =>
      getPropertyById(id, {
        relations: ['user', 'maintenance_task'],
      }),
  );
              ...
              {hasAccess('maintenance_task', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/maintenance-tasks/create?property_id=${data?.id}`}>
                            ...
                            <Td>
                              {hasAccess('maintenance_task', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/maintenance-tasks/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('maintenance_task', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    maintenance_taskHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
